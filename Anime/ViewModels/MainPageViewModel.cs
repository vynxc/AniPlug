...
public partial class MainPageViewModel : ObservableObject
{
    ...
    private async void Init()
    {
        ...
        var collection = new List<AnimeCollectionModel>();
        var moviesCollection = new List<MovieCollectionModel>();
        ...
        var scoreTask = Task.Run(() => MauiProgram.Consumet.GetAnimeSearchAsync(new AnimeSearchConfig { Sort = new List<Sort> { Sort.SCORE_DESC } }));
        var upcomingTask = Task.Run(() => MauiProgram.Consumet.GetAnimeSearchAsync(new AnimeSearchConfig { Status = Status.NOT_YET_RELEASED }));
        var favoriteTask = Task.Run(() => MauiProgram.Consumet.GetAnimeSearchAsync(new AnimeSearchConfig { Sort = new List<Sort> { Sort.FAVOURITES_DESC } }));
        var popularTask = Task.Run(() => MauiProgram.Consumet.GetAnimeSearchAsync(new AnimeSearchConfig { Sort = new List<Sort> { Sort.POPULARITY_DESC } }));
       

        await Task.WhenAll(scoreTask, upcomingTask, favoriteTask, popularTask, trendingTask, recentMoviesTask, recentShowsTask);
